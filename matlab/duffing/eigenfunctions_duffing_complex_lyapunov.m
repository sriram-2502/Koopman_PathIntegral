%% eigenfunctions for duffing system using negative time and plot lyapunov
clc; clear; %close all;
load('level_set.mat')
%load('duffing_lyap.m')
%% system description
...
for x0 = linspace(-2.9, -2.7, ic_pts)
    for y0 = linspace(-2, 2, ic_pts)
        [ts,xs] = ode45(@(t,x)ff(t,x),tspan,[x0 y0]);
        %plot(xs(start_idx:end,1),xs(start_idx:end,2),'k','LineWidth',1); hold on;
        Xs = [Xs;xs(start_idx:end,:)];
    end
...
phi1_real_matlab = 2*real(phi1_matlab);
phi1_imag_matlab = -2*imag(phi1_matlab);

% using complex conjugate pairs as vector
% V_lyap = (phi1_real.^2-phi1_imag.^2)*P(1,1) + (phi1_real.^2-phi1_imag.^2)*P(2,2);
% V_lyap_matlab = (phi1_real_matlab.^2-phi1_imag_matlab.^2)*P(1,1) + ...
%                     (phi1_real_matlab.^2-phi1_imag_matlab.^2)*P(2,2);

V_lyap = P(1,1)*2*phi1_real.^2 + P(2,2)*2*phi1_imag.^2;
...
%V_lyap_matlab = reshape((V_lyap_matlab),size(q2));
...
%scatter(x_zero-1,y_zero,sz,'MarkerEdgeColor','k','MarkerFaceColor','k')
l = streamslice(X,Y,u,v); hold on;
set(l,'LineWidth',1)
set(l,'Color','k');
axes1 = gca;
xlim([-1.75-shift,1.75-shift])
ylim([-1.5,1.5])
axis square
...
% subplot(2,4,2)
% scatter(q1(:),q2(:),sz,log(V_lyap(:)),'filled','MarkerFaceAlpha',alpha); hold on;
% %scatter(x_zero-1,y_zero,sz,'MarkerEdgeColor','k','MarkerFaceColor','k')
% l = streamslice(X,Y,u,v); hold on;
% set(l,'LineWidth',1)
% set(l,'Color','k');
...
subplot(2,4,6)
alpha = 1;
scatter3(q1(:),q2(:),log(V_lyap(:)./10)+11.5,sz,log(V_lyap(:)./10),'filled','MarkerFaceAlpha',alpha); hold on;
l = streamslice(X,Y,u,v); hold on;
%scatter(x_zero-1,y_zero,sz,'MarkerEdgeColor','k','MarkerFaceColor','k')
set(l,'LineWidth',1)
set(l,'Color','k');
...
% V_lyap(r,c) = nan;
% 
% p2 = pcolor(q1,q2,V_lyap./10); hold on;
...
%% save all variables for plotting eifuns on traj
save('duffing_lyap.m')
